...
contract gamblingVault is ERC20("gamblingShare", "gShare"), Ownable{
    ...
    address public gameAddress; // test done 
    ...
    constructor(address _game) { // test done 
        gameAddress = _game;
    }

    modifier onlyGame() { // test done
        require(msg.sender == gameAddress, "Only the game can execute this functions");
        _;
    }

    function deposit() payable external { // test done
        uint256 shareAmount;
        require(msg.value > 0, "Not possible to purchase zero shares"); // test done
        if(totalSupply() == 0) { // test done 
           ...
           shareAmount = msg.value; 
            ...
            shareAmount = (msg.value * totalSupply()) / (address(this).balance - msg.value); // test done (edge cases)
        }
    ...
    function withdraw(uint256 _shares) external returns(uint256) { // test done 
        uint256 amount = (_shares * address(this).balance) / totalSupply(); // test done 
        require(address(this).balance >= reservedAmount + amount, "Not possible due to reserve reservations");
